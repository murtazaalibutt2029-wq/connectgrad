    ...
    <div style={{ background: '#030a1a' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24 }}>
            {stats.map(s => (
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 800, color: '#10b981', marginBottom: 6 }}>{s.value}</div>
      ...
      <section id="how-it-works" style={{ padding: '80px 24px', background: '#030a1a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: '#10b981', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12 }}>How it works</p>
            ...
            {steps.map((step, i) => (
              <div key={step.title} style={{
                ...
                <div style={{
                  ...
                  fontSize: 13, fontWeight: 700, color: '#10b981', marginBottom: 16,
                }}>
                  {i + 1}
                </div>
      ...
      <section style={{ padding: '80px 24px', background: '#071230' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: '#10b981', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12 }}>Pricing</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
            {plans.map(plan => (
              <div key={plan.name} style={{
                ...
                background: plan.highlight ? 'rgba(16,185,129,0.07)' : 'rgba(255,255,255,0.02)',
                border: `1px solid ${plan.highlight ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.06)'}`,
                position: 'relative',
              }}>
                {plan.highlight && (
                  <div style={{
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 2, marginBottom: 8 }}>
                    <span style={{ fontSize: 36, fontWeight: 800, color: plan.highlight ? '#10b981' : '#f1f5f9' }}>{plan.price}</span>
                ...
                <Link to="/signup" style={{
                  ...
                  background: plan.highlight ? 'linear-gradient(135deg, #059669, #10b981)' : 'rgba(255,255,255,0.05)',
      ...
      <section style={{ padding: '80px 24px', background: '#030a1a' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: '#10b981', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12 }}>Employer stories</p>
            ...
            {testimonials.map(t => (
              <div key={t.name} style={{ padding: 28, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16 }}>
                <div style={{ display: 'flex', marginBottom: 8 }}>
                  {[...Array(5)].map((_, i) => <Star key={i} size={13} color="#10b981" fill="#10b981" />)}
                </div>
